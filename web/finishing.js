...
const SiaFinishing=(()=>{
 ...
 function validateBroll(plan,duration){
  ...
  let total=0;plan.forEach((item,i)=>{const overlay=item.type==='text-overlay',length=item.end-item.start;if(!['text-overlay','teaching-card'].includes(item.type)||typeof item.text!=='string'||!item.text.trim())throw Error('Insert '+(i+1)+' needs a supported type and text.');if(!Number.isFinite(item.start)||!Number.isFinite(item.end)||item.start<0||item.end>duration-.15||length<(overlay?1:1.5)||length>(overlay?4:3)||(i&&item.start<plan[i-1].end))throw Error('Insert '+(i+1)+' must last '+(overlay?'1–4':'1.5–3')+' seconds, stay within the voice, and not overlap.');if(!overlay)for(const key of ['visual','purpose','shot','transition','voiceover','captionTreatment','returnToSia'])if(typeof item[key]!=='string'||!item[key].trim())throw Error('Insert '+(i+1)+' is missing '+key+'.');if(item.text.trim().split(/\s+/).length>20)throw Error('Keep insert text to twenty words or fewer.');if(!overlay)total+=length;});if(total>=duration/2)throw Error('Sia must remain visible for most of the video.');return plan;
 }
 ...
 function draw(ctx,video,time,settings,cues,logo,broll=[],terms=[],words=[]){
  ...
  const insert=broll.find(c=>time>=c.start&&time<c.end);if(insert){if(insert.type==='text-overlay')textOverlay(ctx,insert,time,settings);else teachingCard(ctx,insert,time);}
 ...
 async function render({job,captions,settings={},onProgress=()=>{},signal}){
  ...
  try{
   ...
   const {analyser,destination}=stereoAudioGraph(context,audio,loudness.appliedGainDb);const words=job.body.words||[];const samples=new Uint8Array(analyser.fftSize);let heardAudio=false,frames=0,lastTime=0,current=0,startupSync=null;const syncStates=clips.map(()=>({})),syncOrigin=performance.now()/1000;
 ...
 return {render,supportedMime,validateCaptions,validateBroll,captionLines,retentionZoom,stereoAudioGraph,draw,textOverlay,cueWordTimes,startPlayback,playbackSyncDecision,BRAND};
})();
