...
export function validateProduction(script,p){const e=[];if(!p||typeof p!=='object')return ['Prepare the Reel production plan.'];
 ...
 if(!Array.isArray(p.broll)||p.broll.length>3)e.push('Plan zero to three on-screen inserts.');
  ...
  if(!['text-overlay','teaching-card'].includes(b.type))e.push('Insert '+(i+1)+' must be a text-overlay or teaching-card.');
  if(!Number.isFinite(b.start)||!Number.isFinite(b.end)||b.start<0||length<(overlay?1:1.5)||length>(overlay?4:3)||b.end>script.duration||(i&&b.start<p.broll[i-1].end))e.push('Insert '+(i+1)+' needs non-overlapping '+(overlay?'1–4':'1.5–3')+' second timing inside the lesson.');
  ...
  if(!overlay)for(const k of ['visual','purpose','shot','transition','voiceover','captionTreatment','returnToSia'])if(!String(b[k]||'').trim())e.push('Insert '+(i+1)+' needs '+k+'.');});
...
export const ENERGETIC_PRESENTER='Sia is the only person on screen and the only voice. Preserve her exact face, hair, skin tone and outfit from the reference image. Energetic, expressive delivery like a favourite teacher-friend: raised eyebrows and a playful head shake on the mistake, a big genuine smile on the correct line, emphatic hand gestures on key words, leaning toward the camera when she says "Ab aapki baari". Constant natural movement; never frozen or stiff. Chest-up framing, face fully visible, eye contact with the lens, bright even lighting. Accurate lip sync to the supplied audio. No other people, voices, generated text, logos or music.';
export function defaultProduction(v){const b=v.body,duration=Number(b.duration);
 const broll=(Array.isArray(b.overlays)?b.overlays:[]).slice(0,3).map((o,i,all)=>{const length=Math.min(4,Math.max(1,Number(o.seconds)||2.5)),start=+Math.min(Math.max(0,duration-length),duration*(i+1)/(all.length+1)).toFixed(2);return {type:'text-overlay',text:String(o.text||''),mark:OVERLAY_MARKS.includes(o.mark)?o.mark:'',start,end:+(start+length).toFixed(2)};});
...
export function buildProductionPrompt(v,p){const b=v.body;const overlays=(p.broll||[]).map(x=>`- ${x.type==='text-overlay'?(x.mark==='wrong'?'❌ WRONG overlay':x.mark==='right'?'✅ RIGHT overlay':'Text overlay')+' over Sia':'Full-screen teaching card'} at ${x.start}–${x.end}s: "${x.text}"`).join('\n')||'- None. Sia stays full screen throughout.';
...
export function resolveBrollTiming(production,words,duration){
 ...
 return list.map((b,i)=>{const overlay=b.type==='text-overlay',phrase=tokens(b.text).map(norm);let at=-1;for(let k=0;k<=all.length-phrase.length;k++)if(words[k].start>=previousEnd&&phrase.every((t,n)=>t===all[k+n])){at=k;break;}
 const length=overlay?Math.min(4,Math.max(1,b.end-b.start)):Math.min(2.2,Math.max(1.5,b.end-b.start));const gap=overlay?.3:.7;const latest=duration-((list.length-i)*length)-((list.length-i-1)*gap)-.3;
...
export function wordCaptionCues(words,maxWords=3){const cues=[];let chunk=[];const flush=()=>{if(chunk.length)cues.push({start:chunk[0].start,end:chunk.at(-1).end,text:chunk.map(w=>w.word).join(' ')});chunk=[];};
 for(const w of words||[]){chunk.push(w);if(chunk.length>=maxWords||/[.!?,:;]$/.test(w.word))flush();}flush();
